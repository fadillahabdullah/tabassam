...
class Dac01 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$tgl.'","'.$jam.'","'.$spoin.'","'.$tdisplay.'","'.$keterangan.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$tgl."|".$jam."|".$spoin."|".$tdisplay."|".$keterangan);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$spoin = trim(str_replace("'","''",$this->input->post("spoin")));
			$tdisplay = trim(str_replace("'","''",$this->input->post("tdisplay")));
			$keterangan = trim(str_replace("'","''",$this->input->post("keterangan")));
			$operasi = $this->Mdac01->tambah($id, $tgl, $jam, $spoin, $tdisplay, $keterangan);
			if($operasi == "1"){
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$tdisplay = trim(str_replace("'","''",$this->input->post("tdisplay")));
			$keterangan = trim(str_replace("'","''",$this->input->post("keterangan")));
			$operasi = $this->Mdac01->update($id, $tgl, $jam, $spoin, $tdisplay, $keterangan);
			if($operasi == "1"){
				$ket = "ID : $id,\ntgl : $tgl,\nJam : $jam,\nspoin: $spoin,\nID Kandang: $tdisplay,\nKeterangan: $keterangan";
