...
class Col01 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$tgl.'","'.$jam.'","'.$tspoint.'","'.$tactual.'","'.$rh.'","'.$keterangan.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$tgl."|".$jam."|".$tspoint."|".$tactual."|".$rh."|".$keterangan);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$tspoint = trim(str_replace("'","''",$this->input->post("tspoint")));
			$tactual = trim(str_replace("'","''",$this->input->post("tactual")));
			$rh = trim(str_replace("'","''",$this->input->post("rh")));
			$keterangan = trim(str_replace("'","''",$this->input->post("keterangan")));
			$operasi = $this->Mcol01->tambah($id, $tgl, $jam, $tspoint, $tactual, $rh, $keterangan);
			if($operasi == "1"){
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$rh = trim(str_replace("'","''",$this->input->post("rh")));
			$keterangan = trim(str_replace("'","''",$this->input->post("keterangan")));
			$operasi = $this->Mcol01->update($id, $tgl, $jam, $tspoint, $tactual, $rh, $keterangan);
			if($operasi == "1"){
				$ket = "ID : $id,\ntgl : $tgl,\nJam : $jam,\ntspoint: $tspoint,\nID Kandang: $tactual,\nJumlah Telur: $rh,\nKeterangan: $keterangan";
