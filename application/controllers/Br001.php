...
class Br001 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function json(){
        ...
        foreach ($dt as $k){
            ...
            $dtisi .= '["'.$tomboledit.'","'.$id.'","'.$nama.'","'.$st.'","'.$keterangan.'"],';
        }
	...
	public function filter(){
		...
		if(is_array($dt)){
			if(count($dt) > 0){
				...
				echo base64_encode("1|".$id."|".$nama."|".$status."|".$keterangan);
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mbr001->tambah($id, $nama, $status, $keterangan);
			if($operasi == "1"){
				$ket = "ID Gudang: $id,\nNama Gudang: $nama,\nStatus: $status,\nKeterangan: $keterangan";
				$this->Mlog->log_history("Gudang","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$status = trim(str_replace("'","''",$this->input->post("status")));
			$keterangan = trim(str_replace("'","''",$this->input->post("keterangan")));
			$operasi = $this->Mbr001->update($id, $nama, $status, $keterangan);
			if($operasi == "1"){
				$ket = "ID Gudang: $id,\nNama Gudang: $nama,\nStatus: $status,\nKeterangan: $keterangan";
				$this->Mlog->log_history("Gudang","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			...
			if($operasi == "1"){
				...
				$ket = "ID Gudang: $id,\nNama Gudang: $nama,\nStatus: $status,\nKeterangan: $keterangan";
				$this->Mlog->log_history("Gudang","Hapus",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
