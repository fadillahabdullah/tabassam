...
class Jtel1 extends CI_Controller {
	public $idsc;
	public $aksesc = array();
	...
	public function tambah(){
		if($this->aksesc[0] == "1"){
			...
			$operasi = $this->Mjtel1->tambah($id, $nama, $keterangan);
			if($operasi == "1"){
				$ket = "ID jenis: $id,\nNama jenis: $nama,\nStatus: $keterangan";
				$this->Mlog->log_history("jenis","Tambah",$ket);
			}
	...
	public function update(){
		if($this->aksesc[1] == "1"){
			...
			$nama = trim(str_replace("'","''",$this->input->post("nama")));
			$keterangan = trim(str_replace("'","''",$this->input->post("keterangan")));
			$operasi = $this->Mjtel1->update($id, $nama, $keterangan);
			if($operasi == "1"){
				$ket = "ID jenis: $id,\nNama jenis: $nama,\nID Jenis: $keterangan";
				$this->Mlog->log_history("jenis","Update",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
	...
	public function hapus(){
		if($this->aksesc[2] == "1"){
			...
			if($operasi == "1"){
				...
				$ket = "ID jenis: $id,\nNama jenis: $nama,\nID Jenis: $keterangan";
				$this->Mlog->log_history("jenis","Hapus",$ket);
			}
			echo base64_encode($operasi);
		}else{echo base64_encode("99");}
	}
