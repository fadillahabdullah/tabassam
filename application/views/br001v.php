<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Gudang</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">ID Gudang*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtid" placeholder="Masukkan Kode Gudang" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Nama Gudang*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtnama" placeholder="Masukkan Nama Gudang" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
					...
					<div class="form-group col-md-12 jedaobyek">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtketerangan" placeholder="Masukkan Keterangan" autocomplete="off">
					</div>
...
<script>
	...
	function tambah(){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, nama: nama, status: status, keterangan: keterangan},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Gudang Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Gudang Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Gudang Kosong !', icon: 'error'});
            return;
		}
		swal("Memproses Data.....", {button: false, closeOnClickOutside: false, closeOnEsc: false});
		swal({
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Gudang Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
							...
							if(y == 99){
								...
								if(y == 90){
									swal({title: 'Hapus Gagal', text: 'Data Gudang Ini Masih digunakan dalam Data Form Gudang, Sehingga Tidak Dapat di Hapus Hanya Dapat di Ubah', icon: 'error'});
									refresh();
								}else{
