<div class="page-header">
	<h1 class="page-title">Input Daily Check</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						...
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtid" placeholder="Masukkan Kode Dailty Check" autocomplete="off">
					</div>
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">SETT POIN*</label>
						<input type="text" class="form-control jedaobyek" id="txtspoin" placeholder="Input Seet Poin" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Temperature Display*</label>
						<input type="text" class="form-control  jedaobyek" id="txtdisplay" placeholder="Input Temperature" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
...
<script>
	...
	function tambah(){
        ...
        var tgl = $("#txtdate").val();
        var jam = $("#txtjam").val();
        var spoin = $("#txtspoin").val();
        var tdisplay = $("#txtdisplay").val();
        var keterangan = $("#txtket").val();
    
        if(id == "" || tgl == "" || jam == "" || spoin == "" || tdisplay == "" || keterangan == ""){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, tgl: tgl, jam: jam, spoin: spoin, tdisplay: tdisplay, keterangan: keterangan},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Cooling Room Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Cooling Room Kosong !', icon: 'error'});
            return;
		}
		swal("Memproses Data.....", {button: false, closeOnClickOutside: false, closeOnEsc: false});
		swal({
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Cooling Room Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
