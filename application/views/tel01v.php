<div class="page-header">
	<h1 class="page-title">Pengelolaan Data Telur</h1>
			...
			<div class="panel-body">
				<div class="form-row">
                    <div class="form-group col-md-12">
						<label class="form-control-label">ID Telur*</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtid" placeholder="Masukkan Kode Telur" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Tanggal</label>
						<input type="date" class="form-control khusus_abjad jedaobyek" id="txttgl" placeholder="" autocomplete="off">
					</div>
					<div class="form-group col-md-12 jedaobyek">
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label" style="margin-bottom: -5px;">Jenis Telur</label>
						<select class="form-control select2" id="cbjenis">
							<option value="">Silahkan Pilih</option>
							<?php
								if(is_array($dtjenis)){
									if(count($dtjenis)>0){
										foreach($dtjenis as $x){
											echo "<option value='".$x->id."'>".$x->nama."</option>";
										}
									}
								}
					...
					<div class="form-group col-md-12 jedaobyek">
						<label class="form-control-label">Jumlah Butir</label>
						<input type="text" class="form-control khusus_abjad jedaobyek" id="txtket" placeholder="Masukkan Jumlah Butir" autocomplete="off">
					</div>
...
<script>
	...
	function tambah(){
        ...
        var jenistelur = $("#cbjenis").val();
        var keterangan = $("#txtket").val();
    
        if(id == "" || tgl == "" || kandang == "" || jenistelur == "" ){
        ...
        $.ajax({
            ...
            method: "POST",
            data: {id: id, tgl: tgl, kandang: kandang, jenistelur: jenistelur, keterangan: keterangan},
            cache: "false",
            success: function(x){
				swal.close();
				var y = atob(x);
                if(y == 1){
                    swal({
						...
						text: 'Data Telur Berhasil di Tambahkan',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
            ...
            success: function(x){
                ...
                if(y == 1){
                    swal({
						...
						text: 'Data Telur Berhasil di Update',
						icon: 'success'
					}).then((Refreshh)=>{
						refresh();
						tabel.ajax.reload(null, false);
	...
	function hapus(){
        ...
        if(id == ""){
            swal({title: 'Hapus Gagal', text: 'ID Telur Kosong !', icon: 'error'});
            return;
		}
		swal("Memproses Data.....", {button: false, closeOnClickOutside: false, closeOnEsc: false});
		swal({
					...
					success: function(x){
						...
						if(y == 1){
							swal({
								...
								text: 'Data Telur Berhasil di Hapus',
								icon: 'success'
							}).then((Refreshh)=>{
								refresh();
								tabel.ajax.reload(null, false);
							...
							if(y == 99){
								...
								if(y == 90){
									swal({title: 'Hapus Gagal', text: 'Data Telur Ini Masih digunakan dalam Data Form Telur, Sehingga Tidak Dapat di Hapus Hanya Dapat di Ubah', icon: 'error'});
									refresh();
								}else{
